...
The first data set included information about the party affiliation of national politicians. More specifically, it gave the number of governors, senators, and representatives that belonged to each party as well as the president's affiliation on the 15th of each month from January `r pols_df |> pull(year) |> min()` to June `r pols_df |> pull(year) |> max()`. The second data set included the closing values of the Standard & Poor's (S&P) stock market index at the beginning of any given month from January `r snp_df |> pull(year) |> min()` to July `r snp_df |> pull(year) |> max()`. The last data set showed the percentage of unemployment during any given month from Janaury `r unemployment_df |> pull(year) |> min()` to June `r unemployment_df |> pull(year) |> max()`. After tidying and joining all three of these data sets, the resulting data set shows the unemployment percentage (`unemployment_percentage`), the closing S&P stock index (`snp_close`), the party affiliation of the president (`president`), and the number of governors, senators, and representatives who belong to the republican party (`gov_gop`, `sen_gop`, `rep_gop`) and the democratic party (`gov_dem`, `sen_dem`, `rep_dem`). Each of these values are shown for during any given month from January `r all_df |> pull(year) |> min()` to July `r all_df |> pull(year) |> max()`. This data set includes `r nrow(all_df)` observations and `r ncol(all_df)` variables.
