...
well as the president’s affiliation on the 15th of each month from
January 1947 to June 2015. The second data set included the closing
values of the Standard & Poor’s (S&P) stock market index at the
beginning of any given month from January 1950 to July 2015. The last
data set showed the percentage of unemployment during any given month
from Janaury 1948 to June 2015. After tidying and joining all three of
these data sets, the resulting data set shows the unemployment
percentage (`unemployment_percentage`), the closing S&P stock index
(`snp_close`), the party affiliation of the president (`president`), and
the number of governors, senators, and representatives who belong to the
republican party (`gov_gop`, `sen_gop`, `rep_gop`) and the democratic
party (`gov_dem`, `sen_dem`, `rep_dem`). Each of these values are shown
for during any given month from January 1947 to July 2015. This data set
includes 823 observations and 11 variables.
...
data set includes 1188 observations and 15 variables. It describes the
amount of litter collected, type of litter collected, and number of
homes powered (`homes_powered`) on certain dates from 2014 to 2025 for
...
The total weight collected by Professor Trash Wheel was 282.26 tons. The
